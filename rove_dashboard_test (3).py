...
def main():
    ...
    if google_api.previous_reading == current_reading:
        ...
        # 27 Jun 2020 
        # If prob exists after today, replace above code with below code
        # current_reading[0] = current_reading[0].split('km')
        ...
        google_api.sheet.insert_row(current_reading, google_api.numRows+1)
        ...
        ROVE_bot.sendMessage(creds.TELEGRAM_ROVE_GROUP_ID, rove_dashboard_updates) #   ROVE group ID on Telegram.

        # ROVE_bot.sendMessage(creds.TELEGRAM_MY_PERSONAL_ID, rove_dashboard_updates) #  my personal ID on Telegram.
